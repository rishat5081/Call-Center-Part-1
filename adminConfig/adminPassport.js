...
passport.deserializeUser(function (email, done) {
    connection.query('select * from admin_user where admin_email = ?', [email], (error, rows) => {
        if (error)
...
function authenticate_login(req, email, password, done) {
    ...
    connection.query('select * from admin_user where admin_email = ?', [email], (error, rows) => {
        ...
        if (error)
            return done(null, false, { message: 'there is an error at passport :: 34' + error })
        if (!rows.length)
            return done(null, false, { message: 'No username found' })
        if (rows[0].email_veri === 0)
            return done(null, false, { message: 'Please Verify Email' })

        bcrypt.compare(password, rows[0].admin_password, (error, res) => {
            if (res)
...
// as far the admin is directly added by the super admin so the signUp form isn't required until now 
...
// console.log(bcrypt.hashSync('saad1234', 10))
